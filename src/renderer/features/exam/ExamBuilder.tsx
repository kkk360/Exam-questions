  ...
  return (
    <div>
      ...
      {(question.type === 'single_choice' || question.type === 'multiple_choice') && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontWeight: 500, marginBottom: 4 }}>选项</div>
          ...
          <div style={{ marginTop: 8 }}>
            <span style={{ fontWeight: 500, marginRight: 8 }}>答案：</span>
      ...
      {question.type === 'fill_blank' && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontWeight: 500, marginBottom: 4 }}>答案</div>
          {question.blankAnswers.map((ans, i) => (
            <Space key={i} style={{ marginBottom: 6 }}>
              ...
              {question.blankAnswers.length > 1 && (
                <DeleteOutlined onClick={() => update({ blankAnswers: question.blankAnswers.filter((_, j) => j !== i) })} style={{ color: '#999' }} />
              )}
            </Space>
          ))}
      ...
      <div>
        <div style={{ fontWeight: 500, marginBottom: 4 }}>解析（可选）</div>
...
const PaperPreview: React.FC<{
  ...
  return (
    <div style={{ background: '#fff', padding: '40px 36px', fontFamily: '"SimSun","宋体",serif', lineHeight: 1.8, color: '#000', minHeight: '100%' }}>
      <div style={{ textAlign: 'center', borderBottom: '2px solid #000', paddingBottom: 16, marginBottom: 16 }}>
        {schoolName && <div style={{ fontSize: 13, color: '#333' }}>{schoolName}</div>}
        <h1 style={{ fontSize: 20, fontWeight: 'bold', margin: '6px 0', fontFamily: '"SimHei","黑体",sans-serif' }}>{title || '未命名试卷'}</h1>
        {subtitle && <div style={{ fontSize: 13, color: '#333' }}>{subtitle}</div>}
      ...
      {sections.map((section) => (
        <div key={section.id} style={{ marginTop: 20 }}>
          ...
          {section.description && <div style={{ fontSize: 11, color: '#666', marginBottom: 8 }}>{section.description}</div>}
          {section.questions.length === 0 && <div style={{ color: '#bbb', fontSize: 12, padding: '12px 0' }}>暂无题目</div>}
            ...
            return (
              <div key={q.id} style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  ...
                  <span style={{ fontSize: 10, color: '#aaa', whiteSpace: 'nowrap' }}>（{q.score}分）</span>
                </div>
...
const ExamBuilder: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (examId) { await updateExam(examId, data); message.success('已保存') }
      else { const ne = await createExam(data); setExamId(ne.id); message.success('已创建'); navigate(`/exams/${ne.id}/edit`, { replace: true }) }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      {/* Top metadata bar */}
      <div style={{ background: '#fff', padding: '10px 16px', borderBottom: '1px solid #f0f0f0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <Space size="small"><span style={{ fontSize: 13, color: '#666' }}>时长</span>
            <InputNumber min={1} max={300} value={duration} onChange={(v) => setDuration(v || 120)} size="small" style={{ width: 65 }} />
            <span style={{ fontSize: 13, color: '#666' }}>分钟</span></Space>
          <span style={{ fontSize: 13, color: '#666' }}>总分：<strong style={{ color: '#1677ff' }}>{totalScore}</strong></span>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        {/* Left preview */}
        <div style={{ flex: 1, overflow: 'auto', background: '#e8e8e8', padding: 20 }}>
          <div style={{ maxWidth: 700, margin: '0 auto', boxShadow: '0 2px 8px rgba(0,0,0,0.12)', minHeight: 900 }}>
        ...
        {/* Right editor */}
        <div style={{ width: 480, flexShrink: 0, borderLeft: '1px solid #f0f0f0', background: '#fff', overflow: 'auto' }}>
          <div style={{ padding: '10px 16px', borderBottom: '1px solid #f0f0f0', fontWeight: 600, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>出题面板</span>
          </div>
              ...
              items={sections.map((section) => ({
                ...
                label: (
                  <Space onClick={(e) => e.stopPropagation()}>
                    ...
                    <span style={{ color: '#999', fontSize: 12 }}>{section.questions.reduce((s, q) => s + q.score, 0)}分</span>
                  </Space>
                ),
                extra: (
                      ...
                      <div key={q.id} style={{ marginBottom: 6 }}>
                        <div onClick={() => setActiveQuestionId(activeQuestionId === q.id ? null : q.id)}
                          style={{
                            ...
                            background: activeQuestionId === q.id ? '#e6f4ff' : '#fafafa',
                            border: activeQuestionId === q.id ? '1px solid #91caff' : '1px solid #f0f0f0',
                          }}>
                          <MenuOutlined style={{ color: '#ccc' }} />
                          <span style={{ fontWeight: 600, fontSize: 12 }}>{idx + 1}.</span>
                          <span style={{ fontSize: 11, padding: '1px 5px', borderRadius: 3, background: '#f0f0f0', color: '#666' }}>{QUESTION_TYPE_LABELS[q.type]}</span>
                        ...
                        {activeQuestionId === q.id && (
                          <div style={{ padding: '10px 0 4px 24px', borderBottom: '1px solid #f5f5f5' }}>
