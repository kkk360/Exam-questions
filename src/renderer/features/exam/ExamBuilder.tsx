          ...
          {question.options.length < 8 && (
            <Button
              ...
              onClick={() => {
                ...
                update({
                  options: [...question.options, { label, content: '' }]
                })
              }}
            >
              添加选项
      ...
      {question.type === 'fill_blank' && (
        <div style={{ marginBottom: 12 }}>
          ...
          {question.blankAnswers.map((ans, i) => (
            <Space key={i} style={{ marginBottom: 6 }}>
              <Input
                ...
                value={ans}
                onChange={(e) => {
                  const arr = [...question.blankAnswers]
          ...
          <Button
            ...
            size="small"
            icon={<PlusOutlined />}
            onClick={() => update({ blankAnswers: [...question.blankAnswers, ''] })}
          >
            添加备选答案
          </Button>
        </div>
      ...
      {sections.map((section) => (
        <div key={section.id} style={{ marginTop: 20 }}>
          ...
          {section.questions.length === 0 && (
            <div style={{ color: '#d4d4d8', fontSize: 12, padding: '12px 0' }}>暂无题目</div>
          )}
            ...
            return (
              <div key={q.id} style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <RichContent content={q.content || '（请输入题干）'} />
                  </div>
                  <span
                    style={{
                      fontSize: 10,
                ...
                {(q.type === 'single_choice' || q.type === 'multiple_choice') &&
                  q.options.length > 0 && (
                    ...
                    >
                      {q.options.map((opt) => (
                        <div key={opt.label} style={{ fontSize: 13 }}>
                ...
                {q.type === 'fill_blank' && (
                  <div style={{ marginLeft: 24, marginTop: 4, fontSize: 13 }}>
...
const ExamBuilder: React.FC = () => {
        ...
        setSections(
          exam.sections.map((s: any) => ({
            ...
            questions: s.questions.map((sq: any) => {
              ...
              return {
                id: sq.questionId,
                type: q?.type || ('single_choice' as const),
                content: q?.content || '',
                options: q?.options || [],
  ...
  const totalScore = useMemo(
    () => sections.reduce((s, sec) => s + sec.questions.reduce((s2, q) => s2 + q.score, 0), 0),
    [sections]
  )
  ...
  const addQuestion = (sectionId: string, type: PaperQuestion['type']) => {
    ...
    setSections(
      sections.map((s) => (s.id === sectionId ? { ...s, questions: [...s.questions, newQ] } : s))
    )
  ...
  const handlePickerConfirm = async (questionIds: string[], points: number) => {
    ...
    setSections(
      sections.map((s) =>
        s.id === pickerSectionId ? { ...s, questions: [...s.questions, ...newQuestions] } : s
      )
    )
    ...
    setSections(
      sections.map((s) =>
        s.id === sectionId
          ? {
              ...s,
              questions: s.questions.map((pq) => (pq.id === qId ? q : pq))
            }
          : s
      )
    )
  ...
  const saveExam = async (isAuto: boolean) => {
    ...
    try {
      ...
      if (currentId) {
        await updateExam(currentId, data)
        if (!isAuto) message.success('已保存')
      } else {
        const ne = await createExam(data)
        setExamId(ne.id)
        currentId = ne.id
        if (!isAuto) {
          message.success('已创建')
          navigate(`/exams/${ne.id}/edit`, { replace: true })
        }
      }
      if (currentId) {
        const existingKeys = await window.electron.answerKeys.list()
        const existing = existingKeys.find((k: any) => k.examId === currentId)
        if (existing) await window.electron.answerKeys.delete(existing.id)
        const keySections = sections.map((s) => ({
          title: s.title,
          description: s.description,
          items: s.questions.map((q, idx) => {
            let answer = ''
            if (q.type === 'single_choice') answer = String(q.correctAnswer)
            else if (q.type === 'multiple_choice')
              answer = Array.isArray(q.correctAnswer)
        ...
        await window.electron.answerKeys.create({
          examId: currentId,
          examTitle: title || '未命名试卷',
          title: `${title || '未命名'}参考答案`,
          subject,
  ...
  const handleExportPdf = async () => {
    ...
    const fp = await window.electron.export.showSaveDialog(`${title || '试卷'}.pdf`, [
      { name: 'PDF', extensions: ['pdf'] }
    ])
    if (!fp) return
    await window.electron.export.toPdf(examId, fp)
  ...
  const handleExportWord = async () => {
    ...
    const fp = await window.electron.export.showSaveDialog(`${title || '试卷'}.docx`, [
      { name: 'Word', extensions: ['docx'] }
    ])
    if (!fp) return
    await window.electron.export.toWord(examId, fp)
                  ...
                  children: (
                    <div>
                      ...
                      <Space size="small" wrap style={{ marginTop: 8 }}>
                        {(['single_choice', 'multiple_choice', 'fill_blank', 'essay'] as const).map(
                          (t) => (
                            <Button
                              key={t}
                              size="small"
                              type="dashed"
                              icon={<PlusOutlined />}
                              onClick={() => addQuestion(section.id, t)}
                            >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setSections([
                    ...
                    {
                      id: genId(),
                      title: `${names[sections.length] || sections.length + 1}、`,
                      description: '',
                      questions: []
                    }
                  ])
